 ...
 * @returns {*} Promise
 */
export async function processFeedsFromList() {
	const self = this;

	if (self.feedsFromList() && self.feedsFromList().length > 0) {
		return Promise.all(self.feedsFromList().map(async listData => {
			const {listName, listOwner} = listData;
			const listMembers = await self.fetchFeedsFromTwitterList(listName, listOwner);
