...
<<<<<<< HEAD
	...
	return Promise.all(feeds.forEach((feed, index) => {
		new Promise(async resolve => {
=======

	await Promise.all(feeds.map((feed, index) => {
		return new Promise(async resolve => {
>>>>>>> 4cb08969b2e504ff66deab7361de923f1f1d5f05
...
function getCredentials() {
	...
	if (!localConfig) {
		throw new Error('local-config.js is not properly configured or does not exist.');
	}
	return localConfig;
}
