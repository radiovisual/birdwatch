...
module.exports.logStartMessage = function(refreshTime, birdwatch) {
	...
	const feedsFromLists = birdwatch.feedsFromList()
		? birdwatch.feedsFromList().length
		: 0;
	...
	console.log(
		`${chalk.cyan('|')} ${chalk.gray(' Monitoring Feeds: ')} ${chalk.white(
			feeds
		)}`
	);
	console.log(
		`${chalk.cyan('|')} ${chalk.gray(
			' Monitoring Twitter Lists: '
		)} ${chalk.white(feedsFromLists)}`
	);
	console.log(
		`${chalk.cyan('|')} ${chalk.gray(' Refresh Time: ')} ${chalk.white(
			refreshSeconds
		)} secs (${refreshMinutes} min)`
	);
	console.log(
		`${chalk.cyan('|')} ${chalk.gray(' Cache File: ')} ${chalk.white(cache)}`
	);
...
module.exports.reportFetchingMessage = function(screenname, filterTags) {
	const name =
		chalk.white('Fetching twitter data for: ') + chalk.yellow(`@${screenname}`);
