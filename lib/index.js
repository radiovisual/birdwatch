...
export default class Birdwatch {
	...
	async start() {
		...
		if (this.feedsFromList() && this.feedsFromList().length > 0) {
			await Promise.all(self.feedsFromList().map(async listData => {
				const { listName, listOwner, options } = listData;
				const listMembers = await self.fetchFeedsFromTwitterList(listName, listOwner, options);
				const members = await self.listMembersToFeedEntries(listMembers);
				
				members.forEach(member => self.feed(member));
