...
export default class Birdwatch {
	...
	constructor(options) {
		const defaults = {
			logReports: true,
			configFile: path.resolve(__dirname, '..', 'local-config.js'),
			server: true,
			port: 8417,
			testData: false,
			refreshTime: 600,
	...
	async start() {
		...
		if (!self.options.testData && !validFile.sync(self.options.configFile)) {
			throw new Error(`${self.options.configFile} file not found. Cannot connect to Twitter without valid credentials.`);
		}
