...
export default class Birdwatch {
	...
	async start() {
		...
		// ensure we have local-config.js when testData === false
		if (!self.options.testData && !validFile.sync(self.options.configFile)) {
			return Promise.reject(new Error(`${self.options.configFile} file not found. Cannot connect to Twitter without valid credentials.`));
		}

		report.logStartMessage(self.refreshTime, self);
